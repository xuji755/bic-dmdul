...
The first recovery phase is bootstrap:

1. read the copied `dm.ctl` and DBF headers to recover database file structure;
2. find the first SYSTEM tablespace file and the dictionary table locations;
3. dump dictionary information into local files used by later extraction.
